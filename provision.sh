...
if [ -z "$CONFIG_FLAGS" ]; then
	...
	if [ -n "$YARN_VERSION" ]; then
		for server in pgp.mit.edu keyserver.pgp.com ha.pool.sks-keyservers.net; do
			gpg --keyserver $server --recv-keys 6A010C5166006599AA17F08146C2130DFD2497F5 && break;
		done
...
rm -rf ${RM_DIRS} /node-${VERSION}* ~/node-${VERSION}* /usr/share/man /tmp/* /var/cache/apk/* \
